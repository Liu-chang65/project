    ...
    <div className="row">
      <div className="col-md-12">
        {types.map((type) => {
          return (
            <button
              style={{ opacity: deliveryType === type ? 1 : 0.5 }}
              onClick={() => onChange(type)}
              type="button"
              key={type}
              className="px-4 mr-3 btn btn-primary inflex-center-center btn-gray btn-h46"
            >
              {type}
            </button>
          );
...
const ProductModalDetails = ({
  ...
  useEffect(() => {
    ...
    boundSetSelectedPrice(
      mealPrices.filter((price) => price.menuPriceOption === deliveryType)[0],
    );
                          ...
                          <div className="group-price">
                            ...
                            <div className="row py-4">
                              {mealPrices.map((price, index) => {
                                if (price.menuPriceOption !== deliveryType)
                                  return '';
