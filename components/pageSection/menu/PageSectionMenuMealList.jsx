...
const PageSectionMenuMealList = ({
	...
	useEffect(() => {
		...
		if (selectedPageSearch.length > 0 && selectedPageSearch.split("=")[0] === "?initialCurrentPage") {
			let selectedPageNumber = parseInt(selectedPageSearch.split("=")[1]);
			...
			Logger.log(element[0].childNodes, "element------");
			let nodeList = element[0].childNodes;
			nodeList.forEach((list, key) => {
				list.className = ""
				if((selectedPageNumber + 1) === key) {
	...
	const categoriesScrollWheeling = e => {
		...
		if (start) {
			...
			Logger.log('hre', slider, slider.scrollHeight, slider.scrollTop)
	...
	const handleCategoryClick = ({ id }) => {
		...
		// Since it is a new category so we have to start from the page 1
		onPageChange(0);
			...
			<section className="menu-list pd-100">
				...
				<div className="discovery relative" id="discovery" ref={discovery}>
					<div className="sticky-wrapper" style={{ height: `${height}px` }}>
						...
						<div id="rightMain" className="menu-container" style={{ float: 'left', position: 'relative' }}>
							<div className="tab-content tab-content--relative" style={{ position: 'relative'}} id="sticky">
